...
func setup() {
	// Conf
	viper.SetConfigName("tradeFinance") // name of config file (without extension)
...
`)   
        ...
        // This must succeed
        b, err := validateLC(lcJSON)
        err = json.Unmarshal(b, &result)
        if err != nil || result.Result != "Success: The L/C passed all validation rules." {
                t.Fatal(err)
        }


        /* WORKFLOW 1: Start */
        // Happy path: submit lc, accept lc, lc is pos, submit ed, accept ed, receive payment

        // This must succeed
        if err = submitLC(adminCert, "1000", lcJSON, "I", "E", "IB", "EB", []byte(`ICert`), []byte(`ECert`), []byte(`IBCert`), []byte(`EBCert`)); err != nil {
                t.Fatal(err)
        }

        // This must succeed
        b, err = getLCStatus("1000")
        ...
        /*
        // This must succeed
        if err = acceptLC(adminCert, "1000"); err != nil {
                t.Fatal(err)
        } */

        if err = rejectLC(adminCert,"1000","Resend the LC doc with correction"); err != nil {
         	t.Fatal(err)
         }
		
		
		if err = reSubmitLC(adminCert, "1000", lcJSON, "I", "E", "IB", "EB", []byte(`ICert`), []byte(`ECert`), []byte(`IBCert`), []byte(`EBCert`), "Resubmitted by IB"); err != nil {
        ...
        if err != nil || status.Status != "RESUBMITTED_BY_IB" {
                t.Fatal(err)
        }

         // This must succeed
        if err = acceptLC(adminCert, "1000", "LC accepted"); err != nil {
                t.Fatal(err)
        } 
        ...
        //	fmt.Println("status is", status.Status)
                t.Fatal(err)
        }

        // This must succeed
        b, err = validateED("1000", "BL", blJSON)
        err = json.Unmarshal(b, &result)
        if err != nil || result.Result != "Success: All validation checks passed" {
                t.Fatal(err)
        }

        // This must succeed
        b, err = validateED("1000", "INVOICE", invoiceJSON)
        err = json.Unmarshal(b, &result)
        if err != nil || result.Result != "Success: All validation checks passed" {
                t.Fatal(err)
        }
        ...
        // This must succeed
        b, err = validateED("1000", "PACKINGLIST", plJSON)
        err = json.Unmarshal(b, &result)
        if err != nil || result.Result != "Success: All validation checks passed" {
                t.Fatal(err)
        }
        ...
        // This must succeed
        if err = submitED(adminCert, "1000", []byte(``), []byte(``), []byte(``), blJSON, invoiceJSON, plJSON); err != nil {
                t.Fatal(err)
        }
       ...
       // This must succeed
        ...
        // This must succeed
        b, err = getED("1000", "BL", "JSON")
        if err != nil || b == nil {
                t.Fatal(err)
        }

        // This must succeed
        b, err = getED("1000", "INVOICE", "JSON")
        if err != nil || b == nil {
                t.Fatal(err)
        }

        // This must succeed
        b, err = getED("1000", "PACKINGLIST", "JSON")
        if err != nil || b == nil {
                t.Fatal(err)
        }

        // This must succeed
        if err = paymentReceived(adminCert, "1000"); err != nil {
                t.Fatal(err)
        }
        ...
        b, err = getLCStatus("1000")
        err = json.Unmarshal(b, &status)
        if err != nil || status.Status != "PAYMENT_RECEIVED" {
                t.Fatal(err)
        }
       ...
       /* WORKFLOW 2: Start */
        ...
        // Happy path: submit lc, accept lc, lc is not pos, submit ed, accept ed, accept to pay, receive payment
        
        // This must succeed
        if err = submitLC(adminCert, "1001", lcTimeDraftJSON, "I", "E", "IB", "EB", []byte(`ICert`), []byte(`ECert`), []byte(`IBCert`), []byte(`EBCert`)); err != nil {
                t.Fatal(err)
        }

        // This must succeed
        b, err = getLCStatus("1001")
        err = json.Unmarshal(b, &status)
        if err != nil || status.Status != "SUBMITTED_BY_IB" {
                t.Fatal(err)
        }
        ...
        // This must succeed
        if err = acceptLC(adminCert, "1001", "LC accepted"); err != nil {
                t.Fatal(err)
        }
        ...
        // This must succeed
        b, err = validateED("1001", "BL", blJSON)
        err = json.Unmarshal(b, &result)
        if err != nil || result.Result != "Success: All validation checks passed" {
                t.Fatal(err)
        }
        ...
        // This must succeed
        b, err = validateED("1001", "INVOICE", invoiceJSON)
        err = json.Unmarshal(b, &result)
        if err != nil || result.Result != "Success: All validation checks passed" {
                t.Fatal(err)
        }
        ...
        // This must succeed
        b, err = validateED("1001", "PACKINGLIST", plJSON)
        err = json.Unmarshal(b, &result)
        if err != nil || result.Result != "Success: All validation checks passed" {
                t.Fatal(err)
        }
        ...
        // This must succeed
        if err = submitED(adminCert, "1001", []byte(``), []byte(``), []byte(``), blJSON, invoiceJSON, plJSON); err != nil {
                t.Fatal(err)
        }
        ...
        // This must succeed
        if err = acceptED(adminCert, "1001"); err != nil {
                t.Fatal(err)
        }
        ...
        // This must succeed
        if err = acceptToPay(adminCert, "1001"); err != nil {
                t.Fatal(err)
        }
        ...
        b, err = getLCStatus("1001")
        err = json.Unmarshal(b, &status)
        if err != nil || status.Status != "PAYMENT_DUE_FROM_IB_TO_EB" {
                t.Fatal(err)
        }

        // This must succeed
        if err = paymentReceived(adminCert, "1001"); err != nil {
                t.Fatal(err)
        }
        ...
        // Sad path: submit lc, accept lc, lc is not pos, submit ed, accept ed, accept to pay, payment defaulted
        
        // This must succeed
        if err = submitLC(adminCert, "1002", lcTimeDraftJSON, "I", "E", "IB", "EB", []byte(`ICert`), []byte(`ECert`), []byte(`IBCert`), []byte(`EBCert`)); err != nil {
                t.Fatal(err)
        }

        // This must succeed
        b, err = getLCStatus("1002")
        err = json.Unmarshal(b, &status)
        if err != nil || status.Status != "SUBMITTED_BY_IB" {
                t.Fatal(err)
        }
        ...
        // This must succeed
        if err = acceptLC(adminCert, "1002", "LC accepted"); err != nil {
                t.Fatal(err)
        }
        ...
        // This must succeed
        b, err = validateED("1002", "BL", blJSON)
        err = json.Unmarshal(b, &result)
        if err != nil || result.Result != "Success: All validation checks passed" {
                t.Fatal(err)
        }
        ...
        // This must succeed
        b, err = validateED("1002", "INVOICE", invoiceJSON)
        err = json.Unmarshal(b, &result)
        if err != nil || result.Result != "Success: All validation checks passed" {
                t.Fatal(err)
        }
        ...
        // This must succeed
        b, err = validateED("1002", "PACKINGLIST", plJSON)
        err = json.Unmarshal(b, &result)
        if err != nil || result.Result != "Success: All validation checks passed" {
                t.Fatal(err)
        }
        ...
        // This must succeed
        if err = submitED(adminCert, "1002", []byte(``), []byte(``), []byte(``), blJSON, invoiceJSON, plJSON); err != nil {
                t.Fatal(err)
        }

        // This must succeed
        b, err = getEDStatus("1002")
        err = json.Unmarshal(b, &status)
        if err != nil || status.Status != "SUBMITTED_BY_EB" {
                t.Fatal(err)
        }
       ...
       // This must succeed
        if err = acceptED(adminCert, "1002"); err != nil {
                t.Fatal(err)
        }
        ...
        // This must succeed
        if err = acceptToPay(adminCert, "1002"); err != nil {
                t.Fatal(err)
        }
        ...
        // This must succeed
        if err = defaultedOnPayment(adminCert, "1002"); err != nil {
                t.Fatal(err)
        }
        ...
        b, err = getLCStatus("1002")
        err = json.Unmarshal(b, &status)
        if err != nil || status.Status != "PAYMENT_DEFAULTED" {
                t.Fatal(err)
        }
	...
	// Sad path: submit lc, accept lc, lc is not pos, submit ed, BL is PDF, accept ed, payment defaulted
        
	// This must succeed
	if err = submitLC(adminCert, "1003", lcTimeDraftJSON, "I", "E", "IB", "EB", []byte(`ICert`), []byte(`ECert`), []byte(`IBCert`), []byte(`EBCert`)); err != nil {
		t.Fatal(err)
	}

	// This must succeed
	b, err = getLCStatus("1003")
	err = json.Unmarshal(b, &status)
	if err != nil || status.Status != "SUBMITTED_BY_IB" {
		t.Fatal(err)
	}
	...
	// This must succeed
	if err = acceptLC(adminCert, "1003","LC accepted"); err != nil {
		t.Fatal(err)
	}
	...
	// This must succeed
	//b, err = validateED("1003", "BL", blJSON)
	//err = json.Unmarshal(b, &result)
	//if err != nil || result.Result != "Success: All validation checks passed" {
	//	t.Fatal(err)
	//}
	...
	// This must succeed
	b, err = validateED("1003", "INVOICE", invoiceJSON)
	err = json.Unmarshal(b, &result)
	if err != nil || result.Result != "Success: All validation checks passed" {
		t.Fatal(err)
	}
	...
	// This must succeed
	b, err = validateED("1003", "PACKINGLIST", plJSON)
	err = json.Unmarshal(b, &result)
	if err != nil || result.Result != "Success: All validation checks passed" {
		t.Fatal(err)
	}
	...
	// This must succeed
	if err = submitED(adminCert, "1003", []byte(`bl-bytes`), []byte(``), []byte(``), []byte(`{}`), invoiceJSON, plJSON); err != nil {
		t.Fatal(err)
	}

	// This must succeed
	b, err = getEDStatus("1003")
	err = json.Unmarshal(b, &status)
	if err != nil || status.Status != "SUBMITTED_BY_EB" {
		t.Fatal(err)
	}
	...
	// This must succeed
	if err = acceptED(adminCert, "1003"); err != nil {
		t.Fatal(err)
	}
	...
	// This must succeed
	b, err = getED("1003", "BL", "PDF")
	if err != nil || b == nil {
		t.Fatal(err)
	}

	// This must succeed
	b, err = getED("1003", "INVOICE", "JSON")
	if err != nil || b == nil {
		t.Fatal(err)
	}

	// This must succeed
	b, err = getED("1003", "PACKINGLIST", "JSON")
	if err != nil || b == nil {
		t.Fatal(err)
	}

	// This must succeed
	if err = acceptToPay(adminCert, "1003"); err != nil {
		t.Fatal(err)
	}

	// This must succeed
	if err = defaultedOnPayment(adminCert, "1003"); err != nil {
		t.Fatal(err)
	}
	...
	// Sad path: submit lc, accept lc, lc is not pos, submit ed, Invoice is PDF, accept ed, payment defaulted
	
	// This must succeed
	if err = submitLC(adminCert, "1004", lcTimeDraftJSON, "I", "E", "IB", "EB", []byte(`ICert`), []byte(`ECert`), []byte(`IBCert`), []byte(`EBCert`)); err != nil {
		t.Fatal(err)
	}

	// This must succeed
	b, err = getLCStatus("1004")
	err = json.Unmarshal(b, &status)
	if err != nil || status.Status != "SUBMITTED_BY_IB" {
		t.Fatal(err)
	}
	...
	// This must succeed
	if err = acceptLC(adminCert, "1004", "LC accepted"); err != nil {
		t.Fatal(err)
	}
	...
	// This must succeed
	b, err = validateED("1004", "BL", blJSON)
	err = json.Unmarshal(b, &result)
	if err != nil || result.Result != "Success: All validation checks passed" {
		t.Fatal(err)
	}
	...
	// This must succeed
	//b, err = validateED("1004", "INVOICE", invoiceJSON)
	//err = json.Unmarshal(b, &result)
	//if err != nil || result.Result != "Success: All validation checks passed" {
	//	t.Fatal(err)
	//}
	...
	// This must succeed
	b, err = validateED("1004", "PACKINGLIST", plJSON)
	err = json.Unmarshal(b, &result)
	if err != nil || result.Result != "Success: All validation checks passed" {
		t.Fatal(err)
	}
	...
	// This must succeed
	if err = submitED(adminCert, "1004", []byte(``), []byte(`invoice-bytes`), []byte(``), blJSON, []byte(`{}`), plJSON); err != nil {
		t.Fatal(err)
	}

	// This must succeed
	b, err = getEDStatus("1004")
	err = json.Unmarshal(b, &status)
	if err != nil || status.Status != "SUBMITTED_BY_EB" {
		t.Fatal(err)
	}
	...
	// This must succeed
	if err = acceptED(adminCert, "1004"); err != nil {
		t.Fatal(err)
	}
	...
	// This must succeed
	if err = acceptToPay(adminCert, "1004"); err != nil {
		t.Fatal(err)
	}

	// This must succeed
	if err = defaultedOnPayment(adminCert, "1004"); err != nil {
		t.Fatal(err)
	}
	...
	// This must succeed
	if err = submitLC(adminCert, "1005", lcTimeDraftJSON, "I", "E", "IB", "EB", []byte(`ICert`), []byte(`ECert`), []byte(`IBCert`), []byte(`EBCert`)); err != nil {
		t.Fatal(err)
	}

	// This must succeed
	b, err = getLCStatus("1005")
	err = json.Unmarshal(b, &status)
	if err != nil || status.Status != "SUBMITTED_BY_IB" {
		t.Fatal(err)
	}
	...
	// This must succeed
	if err = acceptLC(adminCert, "1005", "LC accepted"); err != nil {
		t.Fatal(err)
	}
	...
	// This must succeed
	b, err = validateED("1005", "BL", blJSON)
	err = json.Unmarshal(b, &result)
	if err != nil || result.Result != "Success: All validation checks passed" {
		t.Fatal(err)
	}
	...
	// This must succeed
	b, err = validateED("1005", "INVOICE", invoiceJSON)
	err = json.Unmarshal(b, &result)
	if err != nil || result.Result != "Success: All validation checks passed" {
		t.Fatal(err)
	}
	...
	// This must succeed
	//b, err = validateED("1005", "PACKINGLIST", plJSON)
	//err = json.Unmarshal(b, &result)
	//if err != nil || result.Result != "Success: All validation checks passed" {
	//	t.Fatal(err)
	//}
	...
	// This must succeed
	if err = submitED(adminCert, "1005", []byte(``), []byte(``), []byte(`pl-bytes`), blJSON, invoiceJSON, []byte(`{}`)); err != nil {
		t.Fatal(err)
	}

	// This must succeed
	b, err = getEDStatus("1005")
	err = json.Unmarshal(b, &status)
	if err != nil || status.Status != "SUBMITTED_BY_EB" {
		t.Fatal(err)
	}
	...
	// This must succeed
	if err = acceptED(adminCert, "1005"); err != nil {
		t.Fatal(err)
	}
	...
	// This must succeed
	if err = acceptToPay(adminCert, "1005"); err != nil {
		t.Fatal(err)
	}

	// This must succeed
	if err = defaultedOnPayment(adminCert, "1005"); err != nil {
		t.Fatal(err)
	}
	...
	// This must succeed
	if err = submitLC(adminCert, "1006", lcTimeDraftJSON, "I", "E", "IB", "EB", []byte(`ICert`), []byte(`ECert`), []byte(`IBCert`), []byte(`EBCert`)); err != nil {
		t.Fatal(err)
	}

	// This must succeed
	b, err = getLCStatus("1006")
	err = json.Unmarshal(b, &status)
	if err != nil || status.Status != "SUBMITTED_BY_IB" {
		t.Fatal(err)
	}
	...
	// This must succeed
	if err = acceptLC(adminCert, "1006", "LC accepted"); err != nil {
		t.Fatal(err)
	}
	...
	// This must succeed
	//b, err = validateED("1006", "BL", blJSON)
	//err = json.Unmarshal(b, &result)
	//if err != nil || result.Result != "Success: All validation checks passed" {
	//	t.Fatal(err)
	//}
	...
	// This must succeed
	//b, err = validateED("1006", "INVOICE", invoiceJSON)
	//err = json.Unmarshal(b, &result)
	//if err != nil || result.Result != "Success: All validation checks passed" {
	//	t.Fatal(err)
	//}
	...
	// This must succeed
	//b, err = validateED("1006", "PACKINGLIST", plJSON)
	//err = json.Unmarshal(b, &result)
	//if err != nil || result.Result != "Success: All validation checks passed" {
	//	t.Fatal(err)
	//}
	...
	// This must succeed
	if err = submitED(adminCert, "1006", []byte(`bl-bytes`), []byte(`invoice-bytes`), []byte(`pl-bytes`), []byte(`{}`), []byte(`{}`), []byte(`{}`)); err != nil {
		t.Fatal(err)
	}

	// This must succeed
	b, err = getEDStatus("1006")
	err = json.Unmarshal(b, &status)
	if err != nil || status.Status != "SUBMITTED_BY_EB" {
		t.Fatal(err)
	}
	...
	// This must succeed
	if err = acceptED(adminCert, "1006"); err != nil {
		t.Fatal(err)
	}
	...
	// This must succeed
	if err = acceptToPay(adminCert, "1006"); err != nil {
		t.Fatal(err)
	}

	// This must succeed
	if err = defaultedOnPayment(adminCert, "1006"); err != nil {
		t.Fatal(err)
	}
	...
	//listLCsByStatus
	llcr, err := listLCsByStatus("PAYMENT_RECEIVED")
	err = json.Unmarshal(llcr, &contractsList)
	if err != nil {
		t.Fatal(err)
	}

	//listLCsByStatus
	llcd, err := listLCsByStatus("PAYMENT_DEFAULTED")
	err = json.Unmarshal(llcd, &contractsList)
	if err != nil {
		t.Fatal(err)
	}
	...
	//listEDsByStatus
	edp, err := listEDsByStatus("PAYMENT_DUE_FROM_IB_TO_EB")
	err = json.Unmarshal(edp, &contractsList)
	if err != nil {
		t.Fatal(err)
	}
...
//validateED
func validateED(UID string, docType string, EDJSON []byte) ([]byte, error) {
        //chaincodeInput := &pb.ChaincodeInput{Function: "validateED", Args: []string{UID, docType, string(EDJSON)}}
        chaincodeInput := &pb.ChaincodeInput{Args: [][]byte{[]byte("validateED"), []byte(UID), []byte(docType), EDJSON}}
...
func validateLC(LCJSON []byte) ([]byte, error) {
        //chaincodeInput := &pb.ChaincodeInput{Function: "validateLC", Args: []string{string(LCJSON)}}
        chaincodeInput := &pb.ChaincodeInput{Args: [][]byte{[]byte("validateLC"), LCJSON}}
...
func submitLC(admCert crypto.CertificateHandler, contractID string, LCJSON []byte, importerName string, exporterName string, importerBankName string, exporterBankName string, importerCert []byte, exporterCert []byte, importerBankCert []byte, exporterBankCert []byte) error {
        ...
        chaincodeInput := &pb.ChaincodeInput{Args: [][]byte{[]byte("submitLC"), []byte(contractID), LCJSON, []byte(importerName), []byte(exporterName), []byte(importerBankName), []byte(exporterBankName), importerCert, exporterCert, importerBankCert, exporterBankCert}}
...
func reSubmitLC(admCert crypto.CertificateHandler, contractID string, LCJSON []byte, importerName string, exporterName string, importerBankName string, exporterBankName string, importerCert []byte, exporterCert []byte, importerBankCert []byte, exporterBankCert []byte, comment string) error {
        // Get a transaction handler to be used to submit the execute transaction
        // and bind the chaincode access control logic using the binding
        ...
        //chaincodeInput := &pb.ChaincodeInput{Function: "submitLC", Args: []string{contractID, string(LCJSON), importerName, exporterName, importerBankName, exporterBankName, string(importerCert), string(exporterCert), string(importerBankCert), string(exporterBankCert)}}
        chaincodeInput := &pb.ChaincodeInput{Args: [][]byte{[]byte("reSubmitLC"), []byte(contractID), LCJSON, []byte(importerName), []byte(exporterName), []byte(importerBankName), []byte(exporterBankName), importerCert, exporterCert, importerBankCert, exporterBankCert, []byte(comment)}}
        chaincodeInputRaw, err := proto.Marshal(chaincodeInput)
        if err != nil {
                return err
        }
...
func getLCStatus(contractID string) ([]byte, error) {
        //chaincodeInput := &pb.ChaincodeInput{Function: "getLCStatus", Args: []string{contractID}}
        chaincodeInput := &pb.ChaincodeInput{Args: [][]byte{[]byte("getLCStatus"), []byte(contractID)}}
...
func acceptLC(admCert crypto.CertificateHandler, contractID string,comment string) error {
        // Get a transaction handler to be used to submit the execute transaction
        // and bind the chaincode access control logic using the binding
        ...
        chaincodeInput := &pb.ChaincodeInput{Args: [][]byte{[]byte("acceptLC"), []byte(contractID), []byte(comment)}}
        chaincodeInputRaw, err := proto.Marshal(chaincodeInput)
        if err != nil {
                return err
        }
...
func rejectLC(admCert crypto.CertificateHandler, contractID string, comment string) error {
        // Get a transaction handler to be used to submit the execute transaction
        // and bind the chaincode access control logic using the binding
        ...
        //chaincodeInput := &pb.ChaincodeInput{Function: "acceptLC", Args: []string{contractID}}
        chaincodeInput := &pb.ChaincodeInput{Args: [][]byte{[]byte("rejectLC"), []byte(contractID), []byte(comment)}}
        chaincodeInputRaw, err := proto.Marshal(chaincodeInput)
        if err != nil {
                return err
        }
...
//submitED
func submitED(admCert crypto.CertificateHandler, contractID string, BLPDF []byte, invoicePDF []byte, packingListPDF []byte, BLJSON []byte, invoiceJSON []byte, packingListJSON []byte) error {
        // Get a transaction handler to be used to submit the execute transaction
        // and bind the chaincode access control logic using the binding
        submittingCertHandler, err := administrator.GetTCertificateHandlerNext()
        if err != nil {
        ...
        chaincodeInput := &pb.ChaincodeInput{Args: [][]byte{[]byte("submitED"), []byte(contractID), BLPDF, invoicePDF, packingListPDF, BLJSON, invoiceJSON, packingListJSON}}
        chaincodeInputRaw, err := proto.Marshal(chaincodeInput)
        if err != nil {
                return err
        }
...
//listLCsByStatus
func listLCsByStatus(status string) ([]byte, error) {
	//chaincodeInput := &pb.ChaincodeInput{Function: "listLCsByStatus", Args: []string{status}}
	chaincodeInput := &pb.ChaincodeInput{Args: [][]byte{[]byte("listLCsByStatus"), []byte(status)}}
